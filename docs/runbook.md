...
`routing.decision`, so the counts above are still one per message. How often do
the two disagree, and on what?
...
`triage_compare`, and `compare_tokens`, what the compared classifier spent. That
is kept out of `total_tokens`, which is what the answers cost. A compared Jev
call is not counted in `jev_answered` or `jev_fell_back`: those stay a record of
what routing did.
